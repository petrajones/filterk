...
kMeansModel(X, nClust):
    ...
    database
filterK(X, Y, model, pdc=90, pnd=90, Nmin=5, Nmin0=1, debugPrint=False):
...
def filterK(X, Y, model, pdc=90, pnd=90, Nmin=5, Nmin0=1, debugPrint=False):
    """
    FilterK applied model formed by filterKModel to database X, Y with
    specified parameters. This function identified set of outliers and
    evaluated degree of outlierness of each sample
    ...
    pdc: percentile of distances to centroid. default value = 90
    pnd: percentile of neighbor distances. default value = 90
    ...
    Outputs
    -------
    model: output of filterKModel and contains list with following
        elements:
            out[0] = object of class 'sklearn.cluster.k_means_.KMeans'
            out[1] = shift
            out[2] = scale
    ...
    to evaluate degree of outlierness
        nearest_cluster_dis: distance to nearest centroid (Low means better)
    ...
    # Oulierness criterion 1: Distance to Centroid Calculations
    ...
    # pdc (90) percentile of distances to centroid
    dist_threshold = np.percentile(nearest_cluster_dist, pdc)
    ...
    percentile_neighbor_dist = np.percentile(distances, pnd, axis=1)

    # Mean distance to neighbours threshold "mdnt" is mean of pnd'th
    # percentile distances to n neighbours
    #mnd_threshold = np.mean(percentile_neighbor_dist)
    mnd_threshold = np.percentile(mean_neighbor_dist, pnd)
